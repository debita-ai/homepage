...
import { FileText, Search, Filter, Download, Copy, QrCode } from "lucide-react";
import { Button } from "@/components/ui/button";
import { Input } from "@/components/ui/input";

interface BoletoBilling {
  id: number;
  customerName: string;
  amount: number;
  dueDate: string;
...
export default function BoletoManagementPage() {
  ...
  const [billings, setBillings] = useState<BoletoBilling[]>([]);
  const [loading, setLoading] = useState(true);

  useEffect(() => {
    const fetchBillings = async () => {
      try {
        const response = await fetch('/api/mock-boleto-management');
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-gray-200">
                ...
                <th className="px-6 py-4 text-left text-sm font-medium text-gray-500">Número do Boleto</th>
              ...
              {loading ? (
                ...
                filteredBillings.map((billing) => (
                  <tr key={billing.id} className="border-b border-gray-100">
                    ...
                    <td className="px-6 py-4">
                      <span className="text-gray-600">{billing.bankName}</span>
                    </td>
                    <td className="px-6 py-4">
                      <span className="text-gray-600">{billing.boletoNumber}</span>
                    </td>
                    <td className="px-6 py-4">
                    ...
                    <td className="px-6 py-4">
                      <div className="flex items-center space-x-2">
                        ...
                        <Button variant="ghost" size="sm" onClick={() => copyToClipboard(billing.barcode)}>
